<?php
/**
 * Deneb functions and definitions
 *
...
if ( ! function_exists( 'deneb_setup' ) ) :
	...
	function deneb_setup() {
		 ...
		 * If you're building a theme based on Deneb, use a find and replace
		 * to change 'deneb' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'deneb', get_template_directory() . '/languages' );
		...
		// Used in slider.
		add_image_size( 'deneb-slider', 1920, 1000, false );

		// Used in hero content.
		add_image_size( 'deneb-hero', 600, 650, false );

		// Used in portfolio, team.
		add_image_size( 'deneb-portfolio', 400, 450, false );

		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary Menu', 'deneb' ),
			'social' => esc_html__( 'Social Menu', 'deneb' ),
		) );
...
function deneb_widgets_init() {
	...
	register_sidebar( array(
		'name'        => esc_html__( 'Sidebar', 'deneb' ),
		'id'          => 'sidebar-1',
		'description' => esc_html__( 'Add widgets here.', 'deneb' ),
		) + $args
	);

	register_sidebar( array(
		'name'        => esc_html__( 'Footer 1', 'deneb' ),
		'id'          => 'sidebar-2',
		'description' => esc_html__( 'Add widgets here to appear in your footer.', 'deneb' ),
		) + $args
	);

	register_sidebar( array(
		'name'        => esc_html__( 'Footer 2', 'deneb' ),
		'id'          => 'sidebar-3',
		'description' => esc_html__( 'Add widgets here to appear in your footer.', 'deneb' ),
		) + $args
	);
	...
	register_sidebar( array(
		'name'        => esc_html__( 'Footer 3', 'deneb' ),
		'id'          => 'sidebar-4',
		'description' => esc_html__( 'Add widgets here to appear in your footer.', 'deneb' ),
		) + $args
	);
...
if ( ! function_exists( 'deneb_fonts_url' ) ) :
	/**
	 * Register Google fonts for Deneb
	 *
	...
	function deneb_fonts_url() {
		...
		*/
		$poppins = _x( 'on', 'Poppins font: on or off', 'deneb' );
...
function deneb_scripts() {
	...
	// Theme stylesheet.
	wp_enqueue_style( 'deneb-style', get_stylesheet_uri(), array(), deneb_get_file_mod_date( 'style.css' ) );

	// Add google fonts.
	wp_enqueue_style( 'deneb-fonts', deneb_fonts_url(), array(), null );

	// Theme block stylesheet.
	wp_enqueue_style( 'deneb-block-style', get_template_directory_uri() . '/css/blocks' . $min . '.css', array( 'deneb-style' ), deneb_get_file_mod_date( 'css/blocks' . $min . '.css' ) );

	$scripts = array(
		'deneb-skip-link-focus-fix' => array(
			'src'      => '/js/skip-link-focus-fix' . $min . '.js',
			'deps'      => array(),
			'in_footer' => true,
		),
		'deneb-keyboard-image-navigation' => array(
	...
	wp_localize_script( 'deneb-script', 'denebScreenReaderText', array(
		'expand'   => esc_html__( 'expand child menu', 'deneb' ),
		'collapse' => esc_html__( 'collapse child menu', 'deneb' ),
	) );
}
...
function deneb_block_editor_styles() {
	// Block styles.
	wp_enqueue_style( 'deneb-block-editor-style', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/editor-blocks.css' );
