...
if ( ! function_exists( 'deneb_breadcrumb' ) ) :
	/**
	 * Display Breadcrumb
	 * @return html Breadcrumb with links
	 */
	function deneb_breadcrumb() {
		...
		$show = deneb_gtm( 'deneb_breadcrumb_show' );
		...
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'deneb' ); // text for the 'Home' link
		/* translators: 1: before text/html, 2: after text/html. */
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'deneb' ); // text for a category page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'deneb' ); // text for a search results page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'deneb' ); // text for a tag page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'deneb' ); // text for an author page
		$text['404']      = esc_html__( '%1$sError 404%2$s', 'deneb' );; // text for the 404 page
		...
		if ( ! is_front_page() ) {
			echo '
			<div id="breadcrumb">
					<div aria-label="' . esc_attr__( 'Breadcrumbs', 'deneb' ) . '" class="breadcrumbs breadcrumb-trail">
			...
			if ( is_home() ) {
				...
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'deneb' ) ) ) ), esc_html( get_the_time( __( 'Y', 'deneb' ) ) ), $content_position++ ) ;

				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'deneb' ) ) ) ), esc_html(get_the_time( __( 'F', 'deneb' ) ) ), $content_position++ );

				echo $before . esc_html( get_the_time( __( 'd', 'deneb' ) ) ) . sprintf( $after, $content_position++ );
			} elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'deneb' ) ) ) ), esc_html( get_the_time( __( 'Y', 'deneb' ) ) ), $content_position++ ) ;

				echo $before . esc_html( get_the_time( __( 'F', 'deneb' ) ) ) . sprintf( $after, $content_position++ );
			} elseif ( is_year() ) {
				echo $before . esc_html( get_the_time( __( 'Y', 'deneb' ) ) ) . sprintf( $after, $content_position++ );
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'deneb' ), absint( max( $paged, $page ) ) );
