...
class Deneb_Customizer {
	...
	public function register_options( $wp_customize ) {
		...
		$section_visibility = Deneb_Customizer_Utilities::section_visibility();

		$section_visibility['excluding-home'] = esc_html__( 'Excluding Homepage', 'deneb' );
		
		unset( $section_visibility['custom-pages'] );

		Deneb_Customizer_Utilities::register_option(
			array(
				'settings'          => 'deneb_header_image_visibility',
				'type'              => 'select',
		...
		$wp_customize->add_section( new Deneb_Upsell_Section( $wp_customize, 'upsell_section',
			array(
				'title'           => esc_html__( 'Deneb Pro Available', 'deneb' ),
				'url'             => 'https://fireflythemes.com/themes/deneb-pro',
	...
	public function preview_js() {
		...
		wp_enqueue_script( 'deneb-customizer', get_template_directory_uri() . '/js/customizer-preview' . $min . '.js', array( 'customize-preview' ), deneb_get_file_mod_date( '/js/customizer-preview' . $min . '.js' ), true );
	}

	/**
	 * Binds the JS listener to make Customizer deneb_color_scheme control.
	 *
	 * @since 1.0
	 */
	public function customize_control_js() {
		...
		// Enqueue Select2.
		wp_enqueue_script( 'deneb-select2-js', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/select2' . $min . '.js', array( 'jquery' ), '4.0.13', true );
		wp_enqueue_style( 'deneb-select2-css', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/select2' . $min . '.css', array(), '4.0.13', 'all' );
		
		// Enqueue Custom JS and CSS.
		wp_enqueue_script( 'deneb-custom-controls-js', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/customizer' . $min . '.js', array( 'jquery', 'jquery-ui-core', 'deneb-select2-js' ), deneb_get_file_mod_date( '/js/customizer' . $min . '.js' ), true );
		
		wp_enqueue_style( 'deneb-custom-controls-css', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/customizer' . $min . '.css', null, deneb_get_file_mod_date( '/css/customizer' . $min . '.css' ), 'all' );
 ...
 */
$deneb_customizer = new Deneb_Customizer();
