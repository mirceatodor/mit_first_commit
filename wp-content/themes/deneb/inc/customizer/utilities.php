...
class Deneb_Customizer_Utilities {
	/**
	...
	public static function register_option( $option ) {
		...
		$defaults = apply_filters( 'deneb_customizer_defaults', array() );
	...
	public static function get_posts_as_array( $args ) {
		...
		if ( ! empty( $get_posts ) ) {
			foreach ( $get_posts as $get_post ) {
				...
				if ( empty( $title ) ) {
					$title = sprintf( esc_html__( 'ID: %s No Title', 'deneb' ), $get_post->ID );
				}
	...
	static function section_visibility() {
		$options = array(
			...
			'homepage'     => esc_html__( 'Homepage / Frontpage', 'deneb' ),
			'entire-site'  => esc_html__( 'Entire Site', 'deneb' ),
		);

		return apply_filters( 'deneb_section_visibility_options', $options );
	}
}
 ...
 * Initialize class
 */
$deneb_utilities = new Deneb_Customizer_Utilities();
