 ...
 * @return array
 */
function deneb_body_classes( $classes ) {
	...
	$classes[] = esc_attr( deneb_gtm( 'deneb_layout_type' ) . '-layout' );
	...
	if ( ! deneb_display_section( $deneb_enable ) || ( ! has_header_image() && ! ( is_header_video_active() && has_header_video() ) ) ) {
    	$classes[] = 'no-header-media';
    }

	return $classes;
}
add_filter( 'body_class', 'deneb_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function deneb_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'deneb_pingback_header' );

if ( ! function_exists( 'deneb_excerpt_length' ) ) :
	/**
	 ...
	 */
	function deneb_excerpt_length( $length ) {
		if ( is_admin() ) {
			return $length;
		}

		// Getting data from Theme Options
		$length	= deneb_gtm( 'deneb_excerpt_length' );

		return absint( $length );
	} // deneb_excerpt_length.
endif;
add_filter( 'excerpt_length', 'deneb_excerpt_length', 999 );

if ( ! function_exists( 'deneb_excerpt_more' ) ) :
	/**
	 * Replaces "[...]" (appended to automatically generated excerpts) with ... and a option from customizer
	 *
	 * @return string option from customizer prepended with an ellipsis.
	 */
	function deneb_excerpt_more( $more ) {
		if ( is_admin() ) {
			return $more;
		}

		$more_tag_text = deneb_gtm( 'deneb_excerpt_more_text' );
...
add_filter( 'excerpt_more', 'deneb_excerpt_more' );

if ( ! function_exists( 'deneb_custom_excerpt' ) ) :
	/**
	 * Adds Continue reading link to more tag excerpts.
	 *
	 * function tied to the get_the_excerpt filter hook.
	 */
	function deneb_custom_excerpt( $output ) {
		if ( is_admin() ) {
			return $output;
		}
		...
		return $output;
	} // deneb_custom_excerpt.
endif;
add_filter( 'get_the_excerpt', 'deneb_custom_excerpt' );

if ( ! function_exists( 'deneb_more_link' ) ) :
	/**
	 * Replacing Continue reading link to the_content more.
	 *
	 * function tied to the the_content_more_link filter hook.
	 */
	function deneb_more_link( $more_link, $more_link_text ) {
		$more_tag_text = deneb_gtm( 'deneb_excerpt_more_text' );

		return str_replace( $more_link_text, wp_kses_data( $more_tag_text ), $more_link );
	} // deneb_more_link.
endif;
add_filter( 'the_content_more_link', 'deneb_more_link', 10, 2 );

/**
 * Filter Homepage Options as selected in theme options.
 */
function deneb_alter_home( $query ) {
	if ( $query->is_home() && $query->is_main_query() ) {
		$cats = deneb_gtm( 'deneb_front_page_category' );
...
add_action( 'pre_get_posts', 'deneb_alter_home' );

/**
 * Display section as selected in theme options.
 */
function deneb_display_section( $option ) {
 ...
 * @return layout
 */
function deneb_get_theme_layout() {
	$layout = '';
	...
	if ( is_page_template( 'templates/full-width-page.php' ) ) {
		...
		$layout = deneb_gtm( 'deneb_default_layout' );

		if ( is_home() || is_archive() ) {
			$layout = deneb_gtm( 'deneb_homepage_archive_layout' );
		}
	}
...
function deneb_get_sidebar_id() {
	...
	if ( $page_id == $page_for_posts || $page_id == $page_on_front ) {
        $sidebaroptions = get_post_meta( $page_id, 'deneb-sidebar-option', true );
    } elseif ( is_singular() ) {
    	if ( is_attachment() ) {
			$parent 		= $post->post_parent;
			$sidebaroptions = get_post_meta( $parent, 'deneb-sidebar-option', true );

		} else {
			$sidebaroptions = get_post_meta( $post->ID, 'deneb-sidebar-option', true );
		}
	}
 ...
 */
function deneb_scrollup() {
	$disable_scrollup = deneb_gtm( 'deneb_band_disable_scrollup' );

	if ( $disable_scrollup ) {
		return;
	}

	echo '<a href="#masthead" id="scrollup" class="backtotop">' . '<span class="screen-reader-text">' . esc_html__( 'Scroll Up', 'deneb' ) . '</span></a>' ;

}
add_action( 'wp_footer', 'deneb_scrollup', 1 );

/**
 * Return args for specific section type
 */
function deneb_get_section_args( $section_name ) {
	$numbers = deneb_gtm( 'deneb_' . $section_name . '_number' );
	...
	for( $i = 0; $i < $numbers; $i++ ) {
		$post__in[] = deneb_gtm( 'deneb_' . $section_name . '_page_' . $i );
	}
 ...
 */
function deneb_button_border_radius() {
	$border_radius = deneb_gtm( 'deneb_button_border_radius' );
	...
	wp_add_inline_style( 'deneb-style', $css );
}
add_action( 'wp_enqueue_scripts', 'deneb_button_border_radius', 11 );

/**
 * Display content.
 */
function deneb_display_content( $section ) {
	?>
 ...
 */
function deneb_display_section_classes( $classes ) {
