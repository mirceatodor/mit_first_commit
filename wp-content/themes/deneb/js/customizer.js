...
jQuery( document ).ready(function($) {
	...
	$('#accordion-section-deneb_ss_main_content h3').unbind("click");
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					denebAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				denebGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			denebGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	function denebAppendRow($element, defaultValue) {
	...
	function denebGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  denebGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function denebGetAllImageCheckboxes($element) {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].denebtinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].denebtinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].denebmediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['deneb-upsell'] = api.Section.extend( {
