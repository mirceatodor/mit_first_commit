...
(function($) {
	...
	function initMainNavigation(container) {
		...
		var dropdownToggle = $('<button />', {
			...
			.append(denebScreenReaderText.icon)
			.append($('<span />', {
				'class': 'screen-reader-text',
				text: denebScreenReaderText.expand
			}));
		...
		container.find('.current-menu-ancestor > button')
			...
			.text(denebScreenReaderText.collapse);
		...
		container.find('.dropdown-toggle').on( 'click', function(e) {
			...
			// jscs:enable
			screenReaderSpan.text(screenReaderSpan.text() === denebScreenReaderText.expand ? denebScreenReaderText.collapse : denebScreenReaderText.expand);
		});
	}
	...
	(function() {
		...
		menuToggle.on('click.deneb', function() {
	...
	(function() {
		...
		function toggleFocusClassTouchScreen() {
			if (window.innerWidth >= 910) {
				$(document.body).on('touchstart.deneb', function(e) {
					if (!$(e.target).closest('.main-navigation li').length) {
						$('.main-navigation li').removeClass('focus');
					}
				});
				siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).on( 'touchstart.deneb', function( e ) {
				...
				siteNavigation.find('.menu-item-has-children > a').unbind('touchstart.deneb');
			}
		}

		if ('ontouchstart' in window) {
			$(window).on('resize.deneb', toggleFocusClassTouchScreen);
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find('a').on('focus.deneb blur.deneb', function() {
	...
	$(document).ready(function() {
		$(window).on('load.deneb resize.deneb', function() {
			if (window.innerWidth < 910) {
